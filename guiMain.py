...
myList = os.listdir(allImgPath)
for cl in myList:
    curImg = cv2.imread(f'{allImgPath}/{cl}')
...
class StartWindow(Screen):
    ...
    def checkAccess(self):
        ...
        if("gst" in self.nik):
            ...
            query = f"select schedule_in,schedule_out,nama from access where nik = {self.nik}"
            staf =True
...
class RecogWindow(Screen,Image):
    ...
    def faceRecognition(self,imgS):
        ...
        try:
            ...
            # print(faceCurrFrame, faceCurrFrame2)
            ...
            for encodeFace,faceloc in zip(encodeCurrFrame,faceCurrFrame):
                ...
                if match[matchIndex]:
                    if(faceDis[matchIndex] < 0.5):
                        nameU = className[matchIndex].upper()
                        name.append(nameU)
                    else:
                        name.append("unknown")
        except Exception as e:
